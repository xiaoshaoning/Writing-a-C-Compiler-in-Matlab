...
**Assembly track (`cc_int.m`): part 7.** Compiles `main` with statements
and local variables to x86-64 COFF assembly (Norasandler series): parts
1-6 expressions (unary, bitwise, logical with short-circuit, comparisons,
arithmetic, parens), part 7 `int x;`/`int x = 5;` declarations (comma
lists), a backpatched local stack frame (`subq`, 16-aligned), expression
statements, and right-associative address-based assignment (`y = x = 5`
chains). Verified end-to-end through gcc; a gcc-gated group in the suite
compiles and runs the `cc2_*.c`–`cc7_*.c` corpus.
...
- **Test suite**: `tests/run_tests.m` — **254/254** on the target runtime
  (146 interpreter checks + 108 gcc-gated assembly-track checks; the gcc
...
```
D:\...\matlab.bat tests/run_tests.m          # full suite (254 checks)
