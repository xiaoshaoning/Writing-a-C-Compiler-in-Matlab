...
tests/
  run_tests.m         test harness (777 checks: probe gate, VM, lexer,
  ...
  programs/           test C programs
    ...
    cc2_*.c–cc19_*.c  unary … hex literals programs (parts 2-18 plus
...
comparisons, and `>>`), and hexadecimal integer literals.
...
`tests/programs/cc2_*.c`–`cc19_*.c` are self-contained examples of each
...
`unsigned`; `cc19_hex.c` covers hexadecimal literals. The full 777-check
...
implemented in the cc18 round, and hexadecimal literals in cc19).
...
Tests (777 checks — probe gate, VM selftest, lexer selftest, the program
