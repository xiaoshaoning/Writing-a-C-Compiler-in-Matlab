...
% --- int64 division keeps the integer class, R2024b half-away-from-zero
%     rounding (DIV-5, fixed in v1.2.40). The port never divides int64
%     directly — cdivmod/word_store use exact double math — but this
%     guards the surface. ---
[npass nfail] = addcheck(npass, nfail, isa(int64(7)/int64(2), 'int64') && ...
                         double(int64(7)/int64(2)) == 4, ...
                         'int64 division keeps class (DIV-5)');
[npass nfail] = addcheck(npass, nfail, double(int64(-7)/int64(2)) == -4, ...
                         'int64 division rounds away from zero (DIV-5)');

% --- int64 arithmetic saturates at INT64_MAX/MIN like real MATLAB (DIV-6
%     contract, fully fixed in v1.2.42). Unreachable for the port (values
%     < 2^53), but guards the surface. ---
[npass nfail] = addcheck(npass, nfail, ...
                         double(int64(2^62) + int64(2^62)) == 9223372036854775807, ...
                         'int64 ADD saturates (DIV-6)');
[npass nfail] = addcheck(npass, nfail, ...
                         double(int64(2^31) * int64(2^32)) == 9223372036854775807, ...
                         'int64 MUL saturates (DIV-6)');
[npass nfail] = addcheck(npass, nfail, ...
                         double(int64(2^63)) == 9223372036854775807, ...
                         'int64(2^63) clamps (DIV-6)');

% --- int64 bitwise ops keep the sign bit (BUG-15, fixed v1.2.45); the VM's
%     OR/XOR/AND depend on this ---
[npass nfail] = addcheck(npass, nfail, double(bitor(int64(0), int64(-1))) == -1, ...
                         'bitor keeps sign bit (BUG-15)');
[npass nfail] = addcheck(npass, nfail, double(bitxor(int64(0), int64(-1))) == -1, ...
                         'bitxor keeps sign bit (BUG-15)');
[npass nfail] = addcheck(npass, nfail, double(bitand(int64(-1), int64(-1))) == -1, ...
                         'bitand keeps sign bit (BUG-15)');
[npass nfail] = addcheck(npass, nfail, ...
                         isa(bitor(uint8([1 2]), uint8([3 4])), 'uint8'), ...
                         'uint8 bitops stay uint8 (DIV-9)');
